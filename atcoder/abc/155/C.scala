object Main extends App {
  ...
  val N = in.nextInt
  val Ss = Array.fill(N)(in.next)
  val targets = Ss.groupBy(identity).mapValues(_.size)
  val max = targets.maxBy(_._2)._2
  println(targets.collect{case (s, `max`) => s}.toArray.sorted.mkString("\n"))
}
